...
*Docker documentation team* &nbsp;·&nbsp; Documentation &nbsp;·&nbsp; Free &nbsp;·&nbsp; 4-6 hours

The official material covers exactly the forty per cent of Docker the chapter asks for: what an image and a container are, layers and why builds are slow, writing a Dockerfile, multi-stage builds, volumes, environment variables, and Compose for local work. Do the "Get started" workshop first, then the [Python language guide](https://docs.docker.com/guides/python/), which walks a FastAPI-style app from Dockerfile to Compose to a GitHub Actions build. Stop there; orchestration can wait until a job asks for it.

> **Worth knowing.** Vendor documentation, so it is thorough on Docker's own tools and quiet on alternatives. The guides assume Docker Desktop, which needs a paid licence at larger companies; the concepts transfer to any container runtime.
