...
The fastest path to running models locally, which matters for beginners for three reasons: zero marginal cost while you iterate, no data leaving your machine, and it forces you to confront quantization and VRAM tradeoffs. 100+ quantized models available. Rule of thumb from 2026 guides: a modern AVX2 CPU with 16GB RAM runs an 8B model at Q4 comfortably. Use Ollama for convenience; drop to llama.cpp directly when you need control over build flags, quantization and sampling.
...
*Percy Liang & Tatsunori Hashimoto* &nbsp;·&nbsp; Course &nbsp;·&nbsp; Free &nbsp;·&nbsp; 150-250 hours

The most rigorous free LLM curriculum in existence, and Spring 2026 lectures are already public. 17 lectures: tokenisation, resource accounting, architectures, attention alternatives + MoE, GPUs/TPUs, Triton kernels, parallelism, scaling laws, inference, evaluation, data sourcing/filtering/dedup/synthetic data, post-training SFT/RLHF, RLVR, alignment + multimodality. 5 assignments (Basics, Systems, Scaling, Data, Alignment & Reasoning RL). This is the ML-Engineer track, not the AI-Engineer track — do it after you can ship something. Do not pay $7,875 as a beginner; the materials are free at github.com/stanford-cs336 and the YouTube playlist.

The [lecture recordings](https://www.youtube.com/playlist?list=PLoROMvodv4rMqXOcazWaTUHhq-yembLCV)
and the [assignment repositories](https://github.com/stanford-cs336) are public as well.

> **Worth knowing.** Not a beginner course: this graduate systems class assumes fluent PyTorch, distributed training and CUDA-adjacent knowledge. Treat it as an end-of-book capstone.
